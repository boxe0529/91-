...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    TabBarController * ta=[[TabBarController alloc]init];
    self.window.rootViewController=ta;
  
    NSArray * paths =NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString * path=paths[0];
    // NSURL * url=[NSURL ur];
    path=[path stringByAppendingPathComponent:@"search.db"];
    ...
    NSString * sql=@"create table if not exists searchs(iid integer,title text)";
